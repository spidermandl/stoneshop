...
jQuery(function(jQuery){
  jQuery(".collection a").click(function(){
    <#if user!?? >
	   ...
	   alert("不能收藏自己的店铺");
	 #else
	   jQuery.post("${S_URL}/add_store_favorite.htm",{"id":"$!store.id"},function(data){
		 if(data==0){
		     alert("店铺收藏成功！");
		 }
		 if(data==1){
  ...
  jQuery("span[id^=ugc_]").css("cursor","pointer").click(function(){
	 ...
	 if(ugc_type=="show"){
	    jQuery(this).find("img").attr("src","${S_URL}/resources/style/shop/$!{store.template}/images/add.jpg");
		jQuery("#ugc_child_"+ugc_id).hide();
	    jQuery(this).attr("ugc_type","hide");
	 }else{
	    jQuery(this).find("img").attr("src","${S_URL}/resources/style/shop/$!{store.template}/images/add2.jpg");
...
<div class="shopindex_left">
  <div class="shoptop">
    ...
    <div class="shopvalue">
      ...
      <ul>
       <li class="shopcusser"><span>站内客服：</span> <span>
              ...
              #else
               <a class="im_common" href="javascript:void(0);" dialog_uri="${S_URL}/user_dialog_login.htm" dialog_title="会员登录" dialog_width="450" dialog_height="100" dialog_id="user_login">咨询客服 </a>
    ...
    <div class="shop_botm_hid">
      <ul class="shopboh">
        <li class="collection"><a href="javascript:void(0);" id="store_fav" #if(!$!user)dialog_uri="${S_URL}/user_dialog_login.htm" dialog_title="会员登录" dialog_width="450" dialog_height="100" dialog_id="user_login"#end><span>收藏店铺</span></a></li>
  ...
  <form action="${S_URL}/store_goods_search.htm" method="post" id="store_search_form">
  ...
  <div class="shopclassify">
    <h1>商品分类</h1>
    <!--箭头sort_a,sort_b-->
    <div class="shop_sort"><a href="${S_URL}/goods_list.htm?store_id=$!{store.id}&orderBy=addTime&orderType=desc">按默认</a><a href="${S_URL}/goods_list.htm?store_id=$!{store.id}&orderBy=store_price&orderType=desc">按价格</a><a href="${S_URL}/goods_list.htm?store_id=$!{store.id}&orderBy=goods_salenum&orderType=desc">按销量</a><a href="${S_URL}/goods_list.htm?store_id=$!{store.id}&orderBy=goods_click&orderType=desc">按人气</a></div>
    <ul class="shopcul">
      <li><span><img src="$!imageWebServer/resources/style/shop/$!{store.template}/images/add2.jpg" width="15" height="15" /></span><a href="${S_URL}/goods_list.htm?store_id=$!{store.id}" class="oneclass">所有商品</a> </li>
      #foreach($ugc in $ugcs)
      #if($!ugc.display)
      <li><span id="ugc_$!{ugc.id}" ugc_type="show" ugc_id="$!ugc.id"><img src="$!imageWebServer/resources/style/shop/$!{store.template}/images/add2.jpg" width="15" height="15" /></span><a href="${S_URL}/goods_list.htm?gc_id=$!{ugc.id}&store_id=$!{store.id}" class="oneclass">$!ugc.className</a> #if($! $ugc.childs.size()>0)
        <ul id="ugc_child_$!{ugc.id}">
          #foreach($cugc in $ugc.childs)
          #if($!cugc.display)
          <li><a href="${S_URL}/goods_list.htm?gc_id=$!{cugc.id}&store_id=$!{store.id}">$!cugc.className</a></li>
          #end
          #end
        </ul>
        #end </li>
  ...
  <div class="shop_rank">
    ...
    <div class="shop_rank_box">
      ...
      <div class="shop_rank_botm" id="sort_sale_goods"> #foreach($goods in $goodsViewTools.sort_sale_goods("$!store.id",5))
        ...
        #end
        #set($goods_url="${S_URL}/goods_$!{goods.id}.htm")
        ...
        <dl>
          ...
          <dd><span class="shop_rank_name"><a href="$!goods_url" target="_blank">$!CommUtil.substring("$!{goods.goods_name}",8)</a></span><span class="shop_rank_money">¥<b>$!{goods.store_price}</b></span><span class="shop_rank_show">出售：<strong>$!{goods.goods_salenum}</strong>笔</span></dd>
        </dl>
        #end </div>
      <div class="shop_rank_botm" style="display:none;" id="sort_collect_goods">
       #foreach($goods in $goodsViewTools.sort_collect_goods("$!store.id",5))
        ...
        <dl>
          ...
          <dd><span class="shop_rank_name"><a href="$!goods_url" target="_blank">$!CommUtil.substring("$!{goods.goods_name}",8)</a></span><span class="shop_rank_money">¥<b>$!{goods.store_price}</b></span><span class="shop_rank_show">收藏：<strong>$!{goods.goods_collect}</strong>次</span></dd>
        </dl>
        #end </div>
    </div>
    <div class="shop_check"><a href="${S_URL}/goods_list.htm?store_id=$!{store.id}" class="shop_check_a">查看本店其他商品</a></div>
  </div>
  <div class="shopf">
    <h1>友情链接</h1>
    <ul>
