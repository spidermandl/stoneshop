...
jQuery(document).ready(function(){
  ...
  jQuery(".left_menu_dl").live("mouseover",function(){
	...
	/*if(jQuery(".left_menu_dd[id=child_"+sc_id+"]").html()==null){
	  jQuery.ajax({type:'POST',url:'$!webPath/nav_data.htm',data:{"sc_id":sc_id},
...
<div class="nav">
	<div class="nav_center">
		<div class="nav_left">
			<h2>
				<a href="${S_URL}/goods_class">
					<span>全部分类<em>
						<img src="${S_URL}/static/styles/system/front/default/images/nav_left.png">
					</em></span>
				</a>
			</h2>
			<div class="index_topleft">
				...
				<#list gcs! as gc >
					<#assign icon_img="${S_URL}/static/images/common/default_icon.png" />
					<dl sc_color="#E60012" style="position:relative;" id="${(gc.id)!}"
		            ...
		            <i class="left_menu_i">
						<img width="16" height="16" src="${icon_img!}">
					</i>
					        ...
					        <div class="menu_con_right_top" style="background-color: rgb(230, 0, 18);">
								<dl><dt>
								  <#list (gc.childs)! as gc1 >
									  ...
									  <span>
										  <a href="${S_URL}/store_goods_list?id=${(gc1.id)!}" target="_blank">${(gc1.classname)!}</a>
									  </span>
									  </#if>
								  </#list>
								</dt></dl>
									...
									<#if ((gc1.display)!'false')=='true' >
										<dl>
											...
											<#list (gc1.childs)! as gc2>
												<#if ((gc1.display)!'false')=='true' >
												  <a href="${S_URL}/store_goods_list?${(gc2.id)!}" target="_blank">${(gc1.classname)!}</a>
												</#if>
											</#list>
											</dd>
										</dl>
    ...
    <#list navs! as nav >
	    ...
	    <li>
			<a id="${(nav.id)!}" <#if ((nav.new_win)!0)==1 >target="_blank"</#if>
