...
<div class="menu-bd-panel">
  <div class="gwc">
    <#if goods_cart!?size gt 0>
    <h1>购物车商品总价：¥<span id="cart_goods_price_top">${total_price!}</span></h1>
    <#list goods_cart! as gc >
    <div class="shopp_ingtop" id="${(gc.id)!}">
      <div class="shopimg">
          <a href="${S_URL}/goods?id=${(gc.goodsId)!}" target="_blank">
          <img src="${RES_URL}/${imageWebServer!}/${(gc.goods.goods_main_photo.path)!}/${(gc.goods.goods_main_photo.name)!}_small.${(gc.goods.goods_main_photo.ext)!}" width="40" height="40" />
          </a>
      </div>
      <div>
          <span class="shopl">
              <a href="${S_URL}/goods?id=${(gc.goodsId)!}" target="_blank">
                  ${CommUtil.substring("${(gc.goods.goodsName)!}",12)!}
      ...
      <div><span class="shopl_del">
          <a href="javascript:void(0);" onclick="cart_remove('${(gc.id)!}','${(gc.goods.goodsStoreId)!}');">删除</a>
      </span></div>
    </div>
    </#list>
    <#else>
