...
//        }
    ...
    public int generic_store_credit(String id){
        int credit = 0;
        String sys_credit = this.systemConfigService.getConfig().getCreditrule();
        Map map = JsonUtils.jsonToPojo(sys_credit,HashMap.class);
        ...
        for (int i = 0; i < ints.length - 1; i++){
            if ((ints[i].intValue() > store.getStoreCredit()) ||
                    (ints[(i + 1)].intValue() < store.getStoreCredit())) continue;
            credit = i + 1;
            break;
        }
    ...
    public int generic_user_credit(String id){
        int credit = 0;
        String user_credit = this.systemConfigService.getConfig().getUser_creditrule();
        Map map = JsonUtils.jsonToPojo(user_credit,HashMap.class);
        List list = new ArrayList();
        for (Iterator it = map.keySet().iterator(); it.hasNext();){
            String key = (String)it.next();
            list.add(Integer.valueOf(Integer.parseInt(map.get(key).toString())));
...
//                      params, 0, count);
    ...
    public List<GsGoods> query_recommend_store_goods(Store store, int begin, int max){
        Map params = new HashMap();
